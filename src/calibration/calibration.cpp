...
void EyeInHandCalibration(double *reprojection_error,
    ...
    *reprojection_error = std::numeric_limits<double>::max();
    ...
    // 여러 알고리즘 다 시도 -> Tsai, Horaud, Andreff, Daniilidis, Park
    for (int method = 0; method < 5; method++) {
        ...
        try {
            ...
            // AX = XB 문제 풀이 (이때 A, B는 포즈가 아닌 모션)
            // A: head2base, B: board2camera, X: eye2head
        ...
        if (!optimization) {
            ...
            std::cout << "optimizaiting ..." << std::endl; // ceres로 비선형 최적화
            ...
            for (size_t i = 0; i < all_corners_2d.size(); i++) {
                for (size_t j = 0; j < all_corners_2d[i].size(); j++) {
                    ...
                    // ExtrinsicsOptimizer, residual dim, q_head2cam, t_head2cam, q_board2base, t_board2base
                    // residual = ||projected_uv - observed_uv|| 2차원 픽셀 오차
            ...
            options.linear_solver_type = ceres::DENSE_SCHUR; // bundle adjustment 문제에 더 적합
            options.max_num_consecutive_invalid_steps = 30;
            options.max_num_iterations = 500; // iteration 500회
...
void EyeInHandOffsetCalibration(double *euclidian_error,
    ...
    for (size_t i = 0; i < computed_3d_rays.size(); i++) {
        ...
        // ExtrinsicsOffsetOptimizer, residual dim, q_head2head_prime, t_head2head_prime
    ...
    // translation 제한 -> x,y,z 각각 -0.1 ~ 0.1m
    ...
    // translation 고정 옵션 -> rotation만 최적화
    ...
    ceres::Solver::Options options;
    options.linear_solver_type = ceres::DENSE_QR; // 변수 적을 때 적합
...
void IntrinsicCalibration(
    ...
    std::cout << "k1: " << D.at<double>(0) << "\n";
    std::cout << "k2: " << D.at<double>(1) << "\n";
    std::cout << "p1: " << D.at<double>(2) << "\n";
    std::cout << "p2: " << D.at<double>(3) << "\n";
    std::cout << "k3: " << D.at<double>(4) << "\n";

    if (D.total() >= 8) {
        std::cout << "k4: " << D.at<double>(5) << "\n";
        std::cout << "k5: " << D.at<double>(6) << "\n";
        std::cout << "k6: " << D.at<double>(7) << "\n";
    }
