...
void CalibrationNode::Init(const std::string cfg_path, bool is_offline, std::string calibration_mode) {
    ...
    if (!cfg_node_["camera"]) {
        ...
        camera_type_ = cfg_node_["camera"]["type"].as<std::string>(); // d-robotics
    ...
    if (calibration_mode_ != "handeye") {
        ...
        if (!cfg_node_["calibration"]["offset"]) {
            ...
            std::string gt_marker_path = cfg_node_["calibration"]["offset"]["field_marker_path"].as<std::string>(); // field.yaml
            exclude_distance_ = cfg_node_["calibration"]["offset"]["exclude_distance"].as<double>(); // 너무 가까운 마커 제거
            ...
            for (auto name : field_node) {
                ...
                cv::Point3f coordinate(coordinate_node[0].as<float>(), coordinate_node[1].as<float>(), 0); // (x, y, 0)
...
void CalibrationNode::RunExtrinsicCalibrationProcess(const SyncedDataBlock &data_block) {
    ...
    if (!is_offline_ && new_log_path_) {
        ...
        data_logger_->LogYAML(cfg_node_, "vision_local.yaml"); // 캘리 전 설정 저장 
        new_log_path_ = false;
    }
    ...
    case 's': {
        ...
        if (found) {
            ...
            if (!is_offline_) {
                data_logger_->LogDataBlock(data_block); // 데이터 파일 저장
            }
        }
        break;
    }
    case 'c': {
        ...
        std::vector<Pose> p_head2bases; // 로봇 head -> base 포즈
        std::vector<Pose> p_board2cameras; // 체커보드 -> 카메라 포즈
        ...
        for (int i = 0; i < cali_data_.size(); i++) {
            ...
            if (board_detector_->DetectBoard(img)) {
                ...
                p_head2bases.push_back(cali_data_[i].pose_data.data); // head -> base 포즈
                p_board2cameras.push_back(board_detector_->getBoardPose()); // 보드 -> 카메라 포즈 (PnP 결과)
        ...
        Pose p_eye2head_best; // 최종 목표 extrinsics
...
void CalibrationNode::RunExtrinsicOffsetCalibrationProcess(const SyncedDataBlock &data_block) {
    ...
    for (auto &detection : detections) {
        ...
        if (detection.class_name.find("Cross") != std::string::npos || detection.class_name == "PenaltyPoint") {
            ...
            cv::Point3f position(xyz[0], xyz[1], xyz[2]); // 추정된 좌표 (bbox 중심으로)

            cv::Point2f uv(detection.bbox.x + detection.bbox.width / 2, detection.bbox.y + detection.bbox.height / 2);
            cv::Point3f ray = intr_.BackProject(uv); // ray 생성
    ...
    case 'c': {
        ...
        for (size_t i = 0; i < marker_coords_vec_.size(); i++) {
            ...
            for (auto &pair : matching) {
                ...
                p_head2bases.push_back(pose); // 이 head pose에서 이 ray는 이 gt_3d를 가리켜야 함

                // 거리 임계값 이내인 것만 선택 (현재는 2)
...
void CalibrationNode::CameraInfoCallback(const sensor_msgs::msg::CameraInfo::SharedPtr msg) {
    ...
    // realsense 카메라는 inverse brown-conrady 모델 사용
    // 다른 카메라는 brown-conrady 모델 사용 
    ...
    if (recevied_count > 10) {
        camera_info_sub_.reset(); // subscription 해제
