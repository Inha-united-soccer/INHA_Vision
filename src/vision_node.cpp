...
void VisionNode::Init(const std::string &cfg_template_path, const std::string &cfg_path) {
    ...
    YAML::Node node = YAML::LoadFile(cfg_template_path); // vision.yaml 파일에 있는 임시 파일
    if (!std::filesystem::exists(cfg_path)) { // /opt/booster에 있는 vision.yaml파일 -> 여기서 수정해야 적용됨
    ...
    // 모델 path 처리 -> 상대경로면 vision 패키지 share 기준으로 절대경로로 변환
    ...
    if (!node["camera"]) {
        ...
        if(camera_type_.empty()){   
            // launch에서 camera_type 안정하면 yaml의 default 사용
        ...
        // head 좌표계를 “보정된 headprime” 같은 것으로 쓰려는 구조일 가능성이 큼.
        p_headprime2head_ = Pose(0, 0, z_comp, 0, pitch_comp * M_PI / 180, yaw_comp * M_PI / 180);

        // 승재욱 추가
        use_imu_ = as_or<bool>(node["use_imu"], false);
    }
    ...
    if (!node["detection_model"]) {
        ...
        if (node["detection_model"]["post_process"]) {
            enable_post_process_ = true; // 후처리 적용 
            single_ball_assumption_ = as_or<bool>(node["detection_model"]["post_process"]["single_ball_assumption"], false); // 공 1개 가정
            if (node["detection_model"]["post_process"]["confidence_thresholds"]) {
                ...
                // 해당 class에 대한 값이 없을 떄 default confidence로 추가 
...
void VisionNode::ProcessData(SyncedDataBlock &synced_data, vision_interface::msg::Detections &detection_msg) {
    ...
    cv::Mat depth_float;
    // depth를 float(m)로 정규화
    if (!depth.empty() && depth.depth() == CV_16U) {
        depth.convertTo(depth_float, CV_32F, 0.001, 0); // 단위 m
    } 
    else {
        depth_float = depth;
    }
    
    // 카메라 -> base link 좌표계 변환
    Pose p_head2base = synced_data.pose_data.data;
    Pose p_eye2base = p_head2base * p_headprime2head_ * p_eye2head_; //  head->base * 헤드 보정 * 외부파라미터

    // 승재욱 추가 -> imu 보정된 좌표계 변환
    if (use_imu_){
        std::cout << "use imu comprehension\n";
        double imu_roll = 0.0;
        double imu_pitch = 0.0;
        {
            std::lock_guard<std::mutex> lock(imu_mutex_);
            if (imu_ready_) {
                imu_roll  = static_cast<double>(imu_rpy_(0)); // roll
                imu_pitch = static_cast<double>(imu_rpy_(1)); // pitch
            }
        }
        Pose imu_comp(0, 0, 0, imu_roll, imu_pitch, 0);
        p_head2base = imu_comp * p_head2base;
    ...
    auto get_estimator = [&](const std::string &class_name) {
        if (class_name == "Ball") { // Ball pose estimator가 있으면 사용, 없으면 default
    ...
    if (enable_post_process_ && !detections.empty()) {
        ...
        if (!confidence_map_.empty()) {
            for (auto &detection : detections) {
                ...
                if (detection.confidence < confidence_map_[classname]) { // confidence 기반 필터링 
                    continue;
                }
        ...
        // 공이 2개 이상 탐지되면 confidence가 가장 높은 것을 선택 
    ...
    for (auto &detection : filtered_detections) {
        ...
        auto pose_estimator = get_estimator(detection.class_name); // 클래스에 맞는 포즈 추정
        Pose pose_obj_by_color = pose_estimator->EstimateByColor(p_eye2base, detection, color); // depth 없이 추정
        Pose pose_obj_by_depth = pose_estimator->EstimateByDepth(p_eye2base, detection, color, depth_float); // depth 이용해서 추정 

        // 공의 depth 추정했는데 Pose() 기본값이면 detection 잘못한 것으로 간주
        ...
        detection_obj.position_projection = pose_obj_by_color.getTranslationVec(); // projection 기반 위치
        detection_obj.position = pose_obj_by_depth.getTranslationVec(); // depth 기반 위치 
        ...
        // color classifier가 있으면 Opponent에 색상 분류기 적용 -> 현재는 안함
        ...
        // publish detection
        detection_msg.detected_objects.push_back(detection_obj); // publish용 msg는 DetectedObject 구조체
        detections_for_display.push_back(detection); // draw용은 DetectionRes 구조체
    }
...
void VisionNode::lowStateCallback(const booster_interface::msg::LowState msg){
    ...
    imu_rpy_(1) = msg.imu_state.rpy[1];
    imu_rpy_(2) = msg.imu_state.rpy[2];

    imu_gyro_(0) = msg.imu_state.rpy[0];
    imu_gyro_(1) = msg.imu_state.rpy[1];
    imu_gyro_(2) = msg.imu_state.rpy[2];

    imu_acc_(0) = msg.imu_state.rpy[0];
    imu_acc_(1) = msg.imu_state.rpy[1];
    imu_acc_(2) = msg.imu_state.rpy[2];

    imu_timestamp_ = this->now().seconds();
    imu_ready_ = true;
}
} // namespace booster_vision
